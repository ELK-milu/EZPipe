...
from modules.Modules.LLM import external_call
...
import keyboard  # 需要安装 keyboard 库，可以使用 pip install keyboard 安装
import asyncio
...
websocket = None
loop = None  # 全局事件循环
text_print = ""  # 全局变量，用于存储最终的识别结果

async def start_recording():
    global is_connected, websocket
    if not is_connected:
        print("w key pressed, starting recording...")
        is_connected = True
        await ws_client(0, 0, 0)  # 启动连接

async def stop_recording():
    global is_connected, websocket, text_print
    if is_connected:
        ...
        print("Sending stop request:", request)
...
# 在 message 函数中更新 text_print
async def message(id):
    global websocket, voices, offline_msg_done, text_print
    text_print_2pass_online = ""
    text_print_2pass_offline = ""
    if args.output_dir is not None:
        ibest_writer = open(
