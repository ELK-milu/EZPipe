...
def cut6(inp):
    ...
    if tmp_str != "":
        opts.append(tmp_str)
    #print(opts)
    if len(opts) > 1 and len(opts[-1]) < 3:  ##如果最后一个太短了，和前一个合一起
        opts[-2] = opts[-2] + opts[-1]
        opts = opts[:-1]
    opts = [item for item in opts if not set(item).issubset(punctuation)]
    return "\n".join(opts)
...
if __name__ == "__main__":
    ...
    seg = []
    tempResponse = ""
    seg = method(tempResponse)
    print(seg)
    print(len(seg.split("\n")))
    cut_msg = seg.split("\n")
    # 若分割出来的句子数量大于1，则返回第一个句子
    if len(cut_msg) > 1:
        # 将self.tempResponse被分割出去的句子从tempResponse中删除
        tempResponse = ''.join(cut_msg[1:])
        sentences = cut_msg[0]
    else:
        # 如果没有分割出句子，则返回空列表
        sentences = []
    print(f"分割后的句子: {sentences}")
    print(f"剩余的文本: {tempResponse}")
